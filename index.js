...
module.exports = function(responderId, config, ss) {
  ...
  ss[name].isConnected = function(sid, cb) {
    conn.hexists(name, sid, cb);
  }

  ss[name].allConnected = function(callback) {
    conn.hkeys(name, function(err, res) {
  ...
  ss[name].purge = function() {
    conn.hgetall(name, function(err, res) {
      var now = Date.now();
      ...
      return {
        websocket: function(msg, meta, send) {
          if(msg == '0') {
            ...
            conn.hset(name, meta.sessionId, Date.now() + (expireDelay * 1000));
          } else if(msg == '1') {
            conn.hexists(name, meta.sessionId, function(err, res) {
